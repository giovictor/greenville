...
if(isset($_POST['editbutton'])) {
	...
	if(empty($publisher)) {
		$updatebookSQL = "UPDATE book SET booktitle='$title', callnumber='$callnumber', classificationID='$classificationID', publisherID=NULL, publishingyear='$year', ISBN='$ISBN',pages='$pages', price='$price' WHERE accession_no='$acc'";	
		$updatebook = mysqli_query($dbconnect, $updatebookSQL);	
	} else if(empty($year)) {
		$updatebookSQL = "UPDATE book SET booktitle='$title', callnumber='$callnumber', classificationID='$classificationID', publisherID='$publisherID', publishingyear=NULL, ISBN='$ISBN',pages='$pages', price='$price' WHERE accession_no='$acc'";	
		$updatebook = mysqli_query($dbconnect, $updatebookSQL);	
	} else if(empty($publisher) && empty($year)) {
		$updatebookSQL = "UPDATE book SET booktitle='$title', callnumber='$callnumber', classificationID='$classificationID', publisherID=NULL, publishingyear=NULL, ISBN='$ISBN',pages='$pages', price='$price' WHERE accession_no='$acc'";	
		$updatebook = mysqli_query($dbconnect, $updatebookSQL);	
	} else {
		$updatebookSQL = "UPDATE book SET booktitle='$title', callnumber='$callnumber', classificationID='$classificationID', publisherID='$publisherID', publishingyear='$year', ISBN='$ISBN',pages='$pages', price='$price' WHERE accession_no='$acc'";	
		$updatebook = mysqli_query($dbconnect, $updatebookSQL);	
	}
	...
	if(empty($author)) {
		...
		$updatebookauthorSQL = "UPDATE bookauthor SET authorID=NULL WHERE accession_no='$acc'";
		...
		$updatebookauthorSQL = "UPDATE bookauthor SET authorID='$authorID' WHERE accession_no='$acc'";
		$updatebookauthor = mysqli_query($dbconnect, $updatebookauthorSQL);
	}

		$titlecode = strtoupper(md5(str_replace(' ','',$title)));
		$newbookID = $titlecode.$authorcode.$publishercode.$yearcode;
		$updatebookIDSQL = "UPDATE book SET bookID='$newbookID' WHERE accession_no='$acc'";
		$updatebookID = mysqli_query($dbconnect, $updatebookIDSQL);

		header("Location:index.php?page=updatebook&acc=$acc");
}
?>