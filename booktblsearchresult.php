...
<div class="admincontainer">
	...
	require "dbconnect.php";
		if(isset($_GET['mngbookbutton'])) {
			if(isset($_GET['mngbooksearch']) && isset($_GET['aedsearchtype'])) {
				...
				$bookSQL = "SELECT bookID, book.accession_no, booktitle, GROUP_CONCAT(DISTINCT author SEPARATOR', ') AS authors , publisher.publisher, callnumber, classification.classificationID, classification.classification, publishingyear, ISBN, book.status, COUNT(DISTINCT book.accession_no) AS copies, price FROM book LEFT JOIN bookauthor ON book.accession_no=bookauthor.accession_no LEFT JOIN author ON author.authorID=bookauthor.authorID LEFT JOIN publisher ON publisher.publisherID=book.publisherID JOIN classification ON classification.classificationID=book.classificationID WHERE classification.classificationID='$classification' AND book.status!='Archived' GROUP BY book.accession_no ORDER BY book.accession_no DESC LIMIT $firstresult, $booksperpages";
			} else if(isset($_GET['startyear']) && isset($_GET['endyear'])) {
				$startyear = mysqli_real_escape_string($dbconnect, htmlspecialchars($_GET['startyear']));
	...
	<div class="table-responsive" id="bookdisplay">
		...
		?>	
				<tr>
					...
					<td>
						<a href="?page=updatebook&bookID=<?php echo $book['accession_no'];?>" class="btn btn-success btn-sm">
	...
	?>
				...
				<?php
					if($page > 1) {
						$previous = $page - 1;
						$pagination .= '<a href="index.php?startyear='.$startyear.'&endyear='.$endyear.'&mngbookbutton=Search&bookpage='.$previous.'">Previous</a>&nbsp;';
		
						for($i = $page - 3; $i < $page; $i++) {
							if($i > 0) {
								$pagination .= '<a href="index.php?startyear='.$startyear.'&endyear='.$endyear.'&mngbookbutton=Search&bookpage='.$i.'">'.$i.'</a>&nbsp;';
							}
						}
					}
					...
					if($page != $numberofpages) {
						$next = $page + 1;
						$pagination .= '<a href="index.php?startyear='.$startyear.'&endyear='.$endyear.'&mngbookbutton=Search&bookpage='.$next.'">Next</a>&nbsp;';	
					}
				?>
	<?php
			}
		...
		$(document).ready(function(){
			...
			?>
					...
					$.ajax({
						url:"booktblfilter.php",
						method:"POST",
						data:{option:option, startyear:startyear, endyear:endyear, booksperpages:booksperpages, firstresult:firstresult},
						success:function(data) {
							$("#bookdisplay").html(data);
						}
					});
				...
				$("#confirmdelete").click(function(){
					...
					$.ajax({
						url:"deletebook.php",
						method:"POST",
						data:{bookid:bookid, option:option, startyear:startyear, endyear:endyear, booksperpages:booksperpages, firstresult:firstresult},
						beforeSend:function() {
							$("#confirmdelete").html("Deleting Book...");
						},
						success:function(data) {
				...
				$(".addbookcopy").click(function(){
					...
					$.ajax({
						url:"addbookcopyinfo.php",
						method:"POST",
						data:{bookID:bookID,startyear:startyear, endyear:endyear, booksperpages:booksperpages, firstresult:firstresult},
						success:function(data) {
							$("#addcopybookdata").html(data);
							$("#addbookcopy").modal("show");
						}
