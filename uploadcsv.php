...
<body>
	...
	<?php
		...
		if(isset($_POST['csvupload'])) {
			...
			$filename = $_FILES['csvtoupload']['name'];
			...
			while($data = fgetcsv($csvuploadfile)) {
				...
				$checkbookSQL = "SELECT COUNT(*) AS noofacc, booktitle FROM book WHERE accession_no='$accession_no'";
				$checkbookQuery = mysqli_query($dbconnect, $checkbookSQL);
				$checkbook = mysqli_fetch_assoc($checkbookQuery);


				if($checkbook['noofacc']==0 && $accession_no!=NULL) {
					if(empty($publisher)) {
						$publisherID = NULL;
						$publishercode = "***";
					} else {
					...
					if(empty($year)) {
						...
						$year = mysqli_real_escape_string($dbconnect,$data[5]);
						$yearcode = mysqli_real_escape_string($dbconnect,$data[5]);
					}
				...
				} else if($checkbook['noofacc']==1) {
					if($checkbook['booktitle']!=$booktitle) {
						$duplicatesSQL = "INSERT INTO duplicate_tables(accession_no, booktitle, filename) VALUES('$accession_no','$booktitle','$filename')";
						$duplicates = mysqli_query($dbconnect, $duplicatesSQL);
					} 
				}
			}
